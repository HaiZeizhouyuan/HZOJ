/*************************************************************************
	...
	> Created Time: 2021年01月15日 星期五 21时37分36秒
 ************************************************************************/
...
int getNext(const char *t, int &j, char input, int *next) {
    while(j != -1 && input != t[j + 1]) j = next[j];
    if (input == t[j + 1]) j += 1;
    return j;
}
...
int kmp(const char *s, const char *t) {
    ...
    next[0] = -1;
    for (int i = 0, j = -1; t[i]; i++) next[i] = getNext(t, j, t[i], next);
...
int sunday(const char *s, const char *t) {
    ...
    for (int i = 0; i < 256; i++) offset[i] = n + 1;//倒数第n+1位.
